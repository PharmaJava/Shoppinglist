-- ══════════════════════════════════════════════════════════════════
-- 0007 · Métricas del panel de administración
--
-- Una sola función que devuelve **todo** el panel en un jsonb. Podrían ser
-- veinte consultas desde el servidor, pero serían veinte viajes de ida y
-- vuelta a Supabase por cada carga de la página; así es uno.
--
-- SEGURIDAD, que aquí es lo delicado: la función lee `auth.users` y agrega
-- datos de todas las listas del sistema, así que se ejecuta como **INVOKER**
-- y sólo se le concede a `service_role`. Con `SECURITY DEFINER` un fallo
-- futuro en los permisos convertiría esto en una fuga de datos de todo el
-- producto; siendo INVOKER, quien la llame sin privilegios no ve nada porque
-- RLS le sigue aplicando. El anon key no puede invocarla.
--
-- No devuelve títulos de listas, ni nombres de usuario, ni correos: el panel
-- es para medir el producto, no para leer las compras de nadie.
-- ══════════════════════════════════════════════════════════════════
...
insert into public.schema_migrations (version) values ('0007_admin_kpis')
